...
boost::shared_ptr< PermutationGroup > AutomorphismComputation::computeRestrictedIsomorphisms ( const Polyhedron& poly ) {
	// we assume that the polyhedron is a cone
	// so the rank of the inequality matrix is at most one less than the 
  ...
  QMatrix* Acopy = new QMatrix(poly.rows(), matrixRank);
	...
	std::list<ulong> polyLinearities = poly.linearities();
	mas.construct(zMatrix, &polyLinearities);
